        ...
        if diamond_exe not in os.listdir(self.dppath):
            ...
            if PanViTaConfig.is_windows():
                diamond_file = FileHandler.safe_download(
                    "https://github.com/bbuchfink/diamond/releases/download/v2.1.24/diamond-windows.zip")
                with zipfile.ZipFile(diamond_file, 'r') as zip_ref:
                    zip_ref.extractall('.')
                diamond_exe = "diamond.exe"
            else:
                diamond_file = FileHandler.safe_download(
                    "https://github.com/bbuchfink/diamond/releases/download/v2.1.24/diamond-linux64.tar.gz")
